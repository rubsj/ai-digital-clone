"""Re-evaluation harness — in-domain only, 3 passes (ADR-017 Phase 1.5.3).

Runs only the 14 in-domain queries through both leaders for 3 passes (84 records).
OOD excluded per ADR-017 Quantified Validation.
Results written to results/evaluation_day12_reeval.json; evaluation_day12.json is
preserved unchanged.

Call-count estimate: 42 pairs × ~14 LLM calls per pair ≈ 588 chat completions
plus ~84 batched embed_openai calls. No new cost-guard pre-flight needed per Phase
1.5.3 plan note (scope smaller than Phase 1 full run, pre-approved envelope).
"""
...
OUTPUT_PATH = Path("results/evaluation_day12_reeval.json")
N_PASSES = 3
