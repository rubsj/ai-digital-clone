...
**Headline finding.** GPT-4o-mini correlates 0.82 with the production embedding-cosine scorer (Pearson=0.8172, p=0.0039) at latency parity — this is the core actionable finding. It supports LLM-based groundedness scoring as a viable alternative to the production scorer where explanation generation and multi-criteria reasoning are also needed. The Ollama-vs-baseline gap is materially larger: Pearson=0.6796 (p=0.0306), a 0.14 deficit relative to GPT. On q03 (binary search), this gap produced a directional disagreement: Ollama=0.9 vs GPT=0.5 vs baseline=0.60 — Ollama's high score likely reflects model knowledge rather than grounded assessment of the retrieved chunks. GPT-vs-Ollama inter-model correlation (Pearson=0.7982) is the least informative of the three: it measures whether they agree with each other, not whether either is calibrated to the production signal. A secondary finding: providing explicit 0.0/0.5/1.0 calibration anchor descriptions caused score quantization (GPT: {0.0, 0.5}; Ollama: {0.0, 0.5, 0.9}), reducing Pearson discriminability. Latency finding from Run 1 (Ollama 2.1x faster) does NOT hold on this harder scoring task — Run 2 shows parity (0.97x).
...
| **Keep?** | GPT-4o-mini: YES for scoring (Pearson=0.82 vs baseline, parity latency). Viable as LLM-based groundedness scorer for both dev and prod. Ollama qwen3:8b: YES for explanation generation (100% structured-output success, viable latency); NO for evaluation scoring where calibration to the production signal matters (Pearson=0.68 vs baseline, 0.14 below GPT; produces directional disagreements on individual queries, e.g. q03). Recommend two ADRs: ADR-007 covers this actionable finding (GPT viable for scoring, Ollama for explanation generation only); ADR-006 covers the methodology-limit cluster (Phases 2, 4, 5 — not this phase). The latency advantage from Run 1 does not hold on the scoring task; run-1 speed (2.1x) was an artifact of the trivial explanation-generation task. |
