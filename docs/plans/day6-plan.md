...
- **Phase 4 (6c) produced a methodology finding, not a weight-sensitivity finding.** 6c did not trigger ADR-006 on its weight-sensitivity criterion (Δ noise-level). However, Phase 4 produced a methodology finding (proxy regime pins style score at ~0.50, std=0.010) that clusters with Phase 2 findings (Cohere bimodal behavior on this corpus; confidence sub-scores pinned under proxy). Phase 7 should consider whether these clustered findings warrant ADR-006 framed as "Evaluation design limits and corpus-shape sensitivity findings on Day 6" rather than as a single weight-change decision. Do not write the ADR yet — Phase 7 is the decision point.

- **Phase 5 (6d) produced a null result on the PRD §8 exit criterion ("measurable shift detected").** However, the underlying finding — that within-individual-email feature variance (std ≈ 0.10–0.21) dominates between-period mean shifts (|Δ| = 0.0002–0.017) — is the third instance of methodology-limit findings on Day 6 (after Phase 2's Cohere bimodal behavior on this corpus, and Phase 4's proxy regime pinning style score). Phase 7 should consider whether this strengthens the case for ADR-006 as a clustered "methodology and corpus-shape findings" framing — three independent experiments, three related observations about measurement-design limits on this corpus and these inputs. Do not write the ADR yet — Phase 7 is the decision point.

- **Phase 6 (6e Run 2) produced a POSITIVE, ACTIONABLE finding that is structurally distinct from the methodology-limit cluster.** GPT-4o-mini correlates 0.82 with the production embedding scorer at latency parity — this is viable for prod and dev. Ollama correlates 0.68, a 0.14 calibration gap that produces directional disagreements on individual queries (q03: Ollama=0.9 vs GPT=0.5 vs baseline=0.60). This is a decision, not a measurement-design limit. **Revised Phase 7 framing: TWO ADRs.**
  - **ADR-006 — Methodology and corpus-shape limits on Day 6.** Covers Phase 2 (Cohere bimodal on this corpus), Phase 4 (proxy regime pins style score), Phase 5 (within-email variance dominates between-period shifts). Theme: measurement-design choices interact with corpus and input characteristics to produce results that look like absence-of-effect but reflect measurement limits. Decision: document as known limits; re-measurement against generated responses (Phase 4) and population-level aggregation (Phase 5) flagged as future work; corpus-shape sensitivity (Phase 2) noted as a carry-forward consideration for ADR-002 amendment. Does NOT include Phase 6 Run 2.
  - **ADR-007 — LLM-based evaluation scoring viability.** Covers Phase 6 Run 2 only. Decision: GPT-4o-mini correlates 0.82 with the production embedding scorer at latency parity — viable substitute for dev and prod where LLM-based scoring's other properties (explanation generation, multi-criteria reasoning) are valuable. qwen3:8b correlates 0.68; the 0.14 gap produces directional disagreements on individual queries. Recommend qwen3:8b for explanation generation where structured-output reliability matters more than score calibration; do NOT recommend for evaluation scoring. Audit trail: Run 1's "Ollama for dev" recommendation was based on a degenerate 2.1x latency advantage; Run 2 found parity on the harder task. Both runs preserved.
