...
Both produce a fixed-length vector that supports cosine similarity.
...
**LLM embeddings (`all-MiniLM-L6-v2`, 384-dim)** - I deferred a clean head-to-head comparison to the Day 6 experiment, but the architectural problem is clear without one. The embedding model was trained on general English text and treats "nak" (kernel NAK, "not acknowledged") as a neutral word with no patch-review semantic. The model can't tell whether high similarity means "wrote like Torvalds" or "wrote about kernel things" — all LKML emails are about kernels, so embedding self-similarity is inflated by topic overlap, not authorial voice. The feature vector separates those: `technical_terminology` captures vocabulary, `patch_language` captures review signals, `capitalization_ratio` captures Torvalds' distinctive ALLCAPS emphasis.
...
I deferred the embedding comparison (MiniLM vs OpenAI) to the Day 6 experiment, where it will be evaluated with retrieval metrics on 10 test queries. This decision relies on the interpretability and domain-specificity arguments above, not on a head-to-head number I haven't cleanly measured yet.
