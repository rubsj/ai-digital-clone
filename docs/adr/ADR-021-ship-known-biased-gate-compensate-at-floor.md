...
The bias is bounded and characterized. Held-equal mean absolute clone score difference is 0.06 with a KH lean on 6 of 7 queries (HHEM); the single Torvalds-leaning held-equal query (q04) confirms the lean is a phrasing effect, not a consistent grounding signal. The oracle inference-label cross-check shows no Torvalds inference lean (Torvalds grounded 87.8% / inferable 12.2%; KH grounded 84.9% / inferable 15.1%). The gate does its routing job: OOD separability AUC 0.942, well clear of the 0.85 bar, so deliver-versus-fallback and the OOD catch are intact.

The operationally meaningful measure is the threshold-relative per-leader deliver-rate gap, measured at W1b.2's `GROUNDEDNESS_MIN = 0.40`. On the held-equal queries the gap is zero (both clones deliver 6/6), because those queries sit in HHEM's polarized zone where a 0.06 score gap up near 0.9 crosses no threshold. On harder held-out queries the bias is real: three Torvalds responses with oracle grounding of 54 to 73 percent score only 0.28 to 0.38 on HHEM and would be wrongly routed to fallback at 0.40. So the raw 0.06 score gap understates the bias on hard queries and overstates it on easy ones; the honest statement is regime-dependent, not "small." The floor compensation itself is pending W3c and is not validated here; it must be pre-registered against this measured, regime-dependent bias, not back-fit to make Torvalds pass.
